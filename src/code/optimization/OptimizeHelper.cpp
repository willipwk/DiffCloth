...
OptimizeHelper::OptimizeHelper(Demos demoNum, Simulation *system,
    ...
    : demoNum(demoNum), system(system), lossInfo(lossInfo), taskInfo(taskInfo),
      lossType(lossType), FORWARD_STEPS(FORWARD_STEPS),
      param_actual(paramActual) {
  ...
  while (true) {
    ...
    if (taskInfo.dL_dcontrolPoints) {
      ...
      for (int s1 = 0; s1 < splineNum; s1++) {
        isValid = isValid && param.controlPointSplines[0][s1].isWithinBound(
                                 system->sceneConfig.sceneBbox);
        if (!isValid)
          break;
...
double OptimizeHelper::operator()(const VecXd &x, VecXd &grad) {
  ...
  Simulation::ParamInfo param = vecXdToParamInfo(x);
  Logging::logColor(
      "LBFGS param:" + Simulation::parameterToString(taskInfo, param) + "\n",
