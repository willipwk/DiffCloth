...
Simulation::isInContactWithObstacle(const Vec3d &pos, const Vec3d &v_in,
  ...
  for (int i = 0; i < primitives.size(); i++) {
    ...
    if (p->isEnabled) {
      // test t=0
      if (p->isInContact(primPos, pos, v_in, info.normal, info.dist,
                         info.v_out)) {
        info.collides = true;
        info.primitiveId = i;
      ...
      if (p->isInContact(primPos, pos + v_in * sceneConfig.timeStep * 0.5, v_in,
                         info.normal, info.dist, info.v_out)) {
        info.collides = true;
        info.primitiveId = i;
      ...
      // test t=timestep
      if (p->isInContact(primPos, pos + v_in * sceneConfig.timeStep, v_in,
                         info.normal, info.dist, info.v_out)) {
        info.collides = true;
        info.primitiveId = i;
...
Simulation::isSelfCollision(const Particle &a, const Particle &b,
                            const Vec3d &x_a, const Vec3d &x_b,
                            const Vec3d &v_a, const Vec3d &v_b) const {
...
#pragma omp parallel for if (OPENMP_ENABLED)
    for (int i = 0; i < particles.size(); i++) {
      ...
      fullCollisionArr[i] = isInContactWithObstacle(
          x_n.segment(p.idx * 3, 3), v.segment(p.idx * 3, 3), x_prim, v_prim);
    }
...
#pragma omp critical
    ...
    if (selfcollisionEnabled) {
      ...
      int cellNum =
          std::max(std::min(512, (int)(dim[axis] / (maxRadii * 2))), 1);
      ...
      for (int i = 0; i < particles.size(); i++) {
        int cellIdx = std::min(
            (int)((x_n.segment(i * 3, 3)[axis] - minDim[axis]) / cellDim),
            cellNum - 1);
...
#pragma omp parallel for if (OPENMP_ENABLED)
      for (int cellId1 = 0; cellId1 < cellNum; cellId1++) {
        for (int cellId2 = cellId1;
             cellId2 < std::min(cellId1 + sweepCellRadius + 2, cellNum);
             cellId2++) {
          for (int p1Idx : cells[cellId1]) {
            for (int p2Idx : cells[cellId2]) {
              ...
              if (!pointpointConnectionTable[p1Idx][p2Idx]) {
                ...
                if (dist > 1.0)
                  continue;
                SelfCollisionInformation info = isSelfCollision(
                    particles[p1Idx], particles[p2Idx], x_i, x_j,
                    v.segment(p1Idx * 3, 3), v.segment(p2Idx * 3, 3));
                if (info.collides) {
                  int infoId = 0;
#pragma omp critical
                  {
                    ...
                    if (selfCollisionMap.find(info.particleId1) ==
                        selfCollisionMap.end()) {
                      selfCollisionMap[info.particleId1] = {};
                    }
                    if (selfCollisionMap.find(info.particleId2) ==
                        selfCollisionMap.end()) {
                    ...
                    selfCollisionTable(info.particleId1, info.particleId2) =
                        infoId;
                    selfCollisionTable(info.particleId2, info.particleId1) =
                        infoId;
                  }
                }
              }
...
void checkInfoConsistency(int particleId, int otherId, int infoIdx,
                          Simulation::SelfCollisionInformation &info) {
  ...
  bool assertion =
      (((info.particleId1 == particleId) && (info.particleId2 == otherId)) ||
...
void removeInfoFromDataStructures(
    int particleId, int otherId, int infoIdx,
    Simulation::SelfCollisionInformation &info,
  ...
  { // map table consistency check
    bool found1 = selfCollisionMap[otherId].find(particleId) !=
                  selfCollisionMap[otherId].end();
    bool found2 = selfCollisionMap[particleId].find(otherId) !=
                  selfCollisionMap[particleId].end();
    bool notCleared = (selfCollisionTable(particleId, otherId) >= 0) &&
                      (selfCollisionTable(otherId, particleId) >= 0);
    if (!found1)
...
std::vector<std::vector<Simulation::SelfCollisionInformation>>
Simulation::contactSorting(Simulation::collisionInfoPair &detections,
                           std::map<int, std::set<int>> &selfCollisionMap,
                           MatXi &selfCollisionTable) {
  std::vector<PrimitiveCollisionInformation> &primitiveCollisions =
      detections.first;
  ...
  // Greedily add lonely selfcollisions ( collisions that look like O-O ) to be
  // processed in one layer
  for (auto it = selfCollisionMap.begin(); it != selfCollisionMap.end(); it++) {
    int particleId = it->first;
    if (selfCollisionMap[particleId].size() != 1)
      continue;
    int otherId = *(it->second.begin());
    if (selfCollisionMap[otherId].size() != 1)
      continue;
    if (frontier.find(particleId) != frontier.end())
      continue;
    if (frontier.find(otherId) != frontier.end())
      continue;
    ...
    removeInfoFromDataStructures(particleId, otherId, infoIdx, info,
                                 selfCollisionMap, selfCollisionTable);
  ...
  while (processedSelfCollisions != selfCollisions.size()) {
#ifdef DEBUG_SELFCOLLISION
    std::printf("*********************\nEXPLOIT NEW FRONTIER: start size "
                "%d\n*********************\n",
                frontier.size());
#endif
    ...
    while (!frontier.empty()) {
#ifdef DEBUG_SELFCOLLISION
      std::printf("====================\nExploring frontier at layer %d:",
                  currentLayer);
      for (int particleId : frontier)
        std::printf("%d ", particleId);
      std::printf("\n");
#endif
      newFrontier.clear();
      ...
      for (int particleId : frontier) {
        if (selfCollisionMap[particleId].empty())
          continue;
        if (involvedInLayer.find(particleId) != involvedInLayer.end())
          continue; // already involved in layer

        for (int otherId : selfCollisionMap[particleId]) {
          // eligibility check
          if (involvedInLayer.find(otherId) != involvedInLayer.end())
            continue; // already involved in layer
          int infoIdx = selfCollisionTable(particleId, otherId);
          SelfCollisionInformation &info = selfCollisions[infoIdx];
          checkInfoConsistency(particleId, otherId, infoIdx, info);
...
#endif
    if (processedSelfCollisions !=
        selfCollisions.size()) { // sorting not finished. Explore new frontier
#ifdef DEBUG_SELFCOLLISION
      ...
      if (frontier.empty()) {
        std::printf("ERROR: NOTHING TO EXPLORE BUT NOT ALL COLLISIONS ARE "
                    "PROCESSED!\n");
        exit(8);
      }
    }
  ...
  for (int i = 0; i < selfCollisions.size(); i++) {
    ...
    if (involvedVertices[layerId].find(selfCollisions[i].particleId1) !=
        involvedVertices[layerId].end()) {
      std::printf("ERROR: particle %d already involved in layerd %d\n",
                  selfCollisions[i].particleId1, layerId);
      exit(8);
    }
    if (involvedVertices[layerId].find(selfCollisions[i].particleId2) !=
        involvedVertices[layerId].end()) {
      std::printf("ERROR: particle %d already involved in layerd %d\n",
...
Simulation::calculateDryFrictionVector(const VecXd &f,
                                       completeCollisionInfo &detectionInfos) {
      ...
      for (std::vector<SelfCollisionInformation> &selfInfos :
        ...
        for (SelfCollisionInformation &info : selfInfos) {
          ...
          Vec3d f_iB = f.segment(info.particleId2 * 3, 3) +
                       r.segment(info.particleId2 * 3, 3);
          ...
          Vec3d r_i =
              k * calcualteDryFrictionForce(info.normal, info.d,
                                            clothFrictionalCoeff, info.type);
          info.r = r_i;
...
Vec3d Simulation::calcualteDryFrictionForce(const Vec3d &n, const Vec3d &f_i,
                                            double mu,
...
// backward r_i --> f_i
Vec3d Simulation::calculatedri_dmu(const Vec3d &n, const Vec3d &f_i,
                                   double mu) const {
  Vec3d dri_dmu;
  dri_dmu.setZero();
...
Mat3x3d Simulation::calculatedri_dfi(const Vec3d &n, const Vec3d &f_i,
                                     double mu) const {
...
double Simulation::stepFixPoints(double t) {
  ...
  case TrajectoryConfigs::FIXED_POINT_TRAJECTORY: {
    for (int fixedPointIdx = 0;
         fixedPointIdx < sysMat[currentSysmatId].fixedPoints.size();
         fixedPointIdx++) {
      sysMat[currentSysmatId].fixedPoints[fixedPointIdx].pos =
          fixedPointTrajectory[forwardRecords.size() - 1].segment(
              fixedPointIdx * 3, 3);
    }
    break;
  }
  ...
  case TrajectoryConfigs::TRAJECTORY_DRESS_TWIRL: {
    ...
    for (int fixedPointIdx = 0;
      ...
      sysMat[currentSysmatId].fixedPoints[fixedPointIdx].pos =
          relativePosRot +
          rotationCenter; // posLast.cross(Vec3d(0, 1, 0)) * 0.012;
    }
  ...
  case TrajectoryConfigs::PER_STEP_TRAJECTORY: {
    for (int fixedPointIdx = 0;
         ...
         fixedPointIdx++) {
      sysMat[currentSysmatId].fixedPoints[fixedPointIdx].pos =
          rlFixedPointPos.segment(fixedPointIdx * 3, 3);
    }
  ...
  default: {
    for (Spline &s : sysMat[currentSysmatId].controlPointSplines) {
      sysMat[currentSysmatId].fixedPoints[s.pFixed].pos =
          s.evalute(t_splinefraction);
    }
    break;
  }
  }
...
void Simulation::stepNN(int idx, const VecXd &x, const VecXd &v,
  ...
  if (fixedPointDofs != requiredDofs) {
    Logging::logWarning("require" + std::to_string(requiredDofs) +
                        " fixed point dofs but input fixed point dof is " +
...
Vec3d Simulation::getInitParticlePos(int i, int j) const {
  ...
  double gridSizeX =
      sceneConfig.fabric.clothDimX / (sceneConfig.fabric.gridNumX - 1);
  double gridSizeY =
      sceneConfig.fabric.clothDimY / (sceneConfig.fabric.gridNumY - 1);
  Vec3d origin =
      Vec3d(-(sceneConfig.fabric.gridNumY - 1) / 4.0 * gridSizeY, 15, 0);
...
void Simulation::initScene() {
  ...
  case FOOT: {
    sockLeg.createNewMesh(Vec3d(0, 0, 0),
                          sceneConfig.sockLegOrientation.normalized(), 5, 4),
  ...
  std::function<void(std::vector<Primitive *> &)> enablePrimitives =
      [&enablePrimitives](std::vector<Primitive *> &prims) {
  ...
  case BIG_SPHERE: {
    Vec3d centerLowPoint = 0.5 * (restShapeMinDim + restShapeMaxDim);
    centerLowPoint[1] = restShapeMinDim[1];
    veryBigSphere.center = veryBigSphere.centerInit =
        Vec3d(-0.50, -16.00, 0.00);
  ...
  case PLANE_BUST_WEARHAT: {
    ...
    plane1.center = plane1.centerInit =
        centerLowPoint - Vec3d(0, 0.5, 0) - Vec3d(0, 0, 4);
    sphere_head.mu = 0.1;
    sphere_head.rotates = sphere_head.gravityEnabled = false;
    sphere_head.isStaitc = true;
    plane1.mu = 0.8;
    sphere_head.center = sphere_head.centerInit =
        plane1.center + Vec3d(0, sphere_head.radius + 0.5, -4);
    plane1.center[1] -= 2;
    break;
  }
  ...
  case CORNERS_2_WEARHAT: {
    {
      ...
      if (sceneConfig.primitiveConfig != PLANE_BUST_WEARHAT) {
        std::printf("ERROR: Trajectory setting is CORNERS_2_WEARHAT but "
                    "primitive config is not PLANE_BUST_WEARHAT\n");
        break;
      }

      Vec3d bustCenter =
          sphere_head.center + Vec3d(0, sphere_head.radius * 0.6, 0);
  ...
  case CORNERS_2_WEARSOCK: {
    {
      if ((sysMat[0].fixedPoints.size() < 2) ||
          (sysMat[0].controlPointSplines.size() < 2)) {
        std::printf("ERROR: Trajectory setting is CORNERS_2_WEARSOCK but there "
                    "are <2 control splines\n");
        break;
      }

      if (sceneConfig.primitiveConfig != FOOT) {
        std::printf("ERROR: Trajectory setting is CORNERS_2_WEARSOCK but "
                    "primitive config is not FOOT\n");
        break;
      }
      ...
      for (Spline &s : sysMat[0].controlPointSplines) {
        s.segments[0].yUp = -28;
        s.moveEndPoint(0,
                       sysMat[0].fixedPoints[s.pFixed].pos_rest + translation);
      }
...
void Simulation::createBendingConstraints() {
  ...
  for (int triIdx = 0; triIdx < mesh.size(); triIdx++) {
    ...
    for (int v1Idx = 0; v1Idx < 3; v1Idx++) {
      for (int v2Idx = v1Idx + 1; v2Idx < 3; v2Idx++) {
        int minIdx = std::min(t.idxArr[v1Idx], t.idxArr[v2Idx]),
            maxIdx = std::max(t.idxArr[v1Idx], t.idxArr[v2Idx]);
  ...
  for (auto const &[vertexPair, idxArr] : edgeTriangleMap) {
    if (idxArr.size() > 1) {
      if (idxArr.size() > 2) {
        std::printf(" ERROR: Detected an edge shared with more than two "
                    "triangles. This is a non-manifold!\n");
        exit(5);
      }
      bendingConstraints.emplace_back(vertexPair.first, vertexPair.second,
                                      idxArr[0], idxArr[1], particles);
    }
  }
}
...
std::vector<Vec3d>
Simulation::rotatePointsAroundCenter(std::vector<Vec3d> &particleIn,
                                     Rotation rotation) {
...
void Simulation::createClothMeshFromModel(std::vector<Vec3d> &particleIn,
                                          std::vector<Vec3i> &meshIn) {
  if (particles.size() > 20000) {
    std::printf("Model has %zu vertices and %zu triangles. The program might "
                "take a long time to run\n",
                particleIn.size(), meshIn.size());
  }
  ...
  Vec3d translation = sceneConfig.fabric.keepOriginalScalePoint
                          ? Vec3d(0, 0, 0)
                          : restShapeMaxDim / 2.0;
  ...
  for (Vec3d &p : particleIn) {
    if (std::isnan(p[0]) || std::isnan(p[1]) || std::isnan(p[2])) {
      std::printf("encountered NAN pos (%.3f, %.3f, %.3f) when loading vertex "
                  "number %zu from file\n",
                  p[0], p[1], p[2], particles.size());
    }
    Vec3d normalizedPos = sceneConfig.fabric.keepOriginalScalePoint
                              ? p
                              : (p - minDim) / scale - restShapeMaxDim;
  ...
  pointpointConnectionTable.resize(particles.size(),
                                   std::vector<bool>(particles.size(), false));
...
void Simulation::createAttachments(bool isModel) {
  if (isModel) {
    ...
    case LEFT_RIGHT_CORNERS_2: {
      // Add attachments
      Vec3d middleUpGoalPoint = Vec3d(
          (restShapeMinDim[0] + restShapeMaxDim[0]) / 2.0, restShapeMaxDim[1],
          (restShapeMinDim[2] + restShapeMaxDim[2]) / 2.0);
      Vec3d upperLeftGoalPoint =
          Vec3d(restShapeMinDim[0], restShapeMaxDim[1],
                (restShapeMinDim[2] + restShapeMaxDim[2]) / 2.0);
      Vec3d upperRightGoalPoint =
          Vec3d(restShapeMaxDim[0], restShapeMaxDim[1],
                (restShapeMinDim[2] + restShapeMaxDim[2]) / 2.0);
      Vec3d lowerLeftGoalPoint =
          Vec3d(restShapeMinDim[0], restShapeMinDim[1],
                (restShapeMinDim[2] + restShapeMaxDim[2]) / 2.0);
      Vec3d lowerRightGoalPoint =
          Vec3d(restShapeMaxDim[0], restShapeMinDim[1],
                (restShapeMinDim[2] + restShapeMaxDim[2]) / 2.0);
      ...
      for (Particle &p : particles) {
        for (std::pair<Vec3d, Particle *> &search : goalPoints) {
          ...
          if ((p.pos_rest - goalPos).norm() <
              (search.second->pos_rest - goalPos).norm()) {
            search.second = &p;
          }
        }
      }
      ...
      for (std::pair<Vec3d, Particle *> &search : goalPoints) {
        ...
        sysMat[0].fixedPoints.emplace_back(bestParticle->pos_rest,
                                           sysMat[0].fixedPoints.size());
        sysMat[0].attachments.emplace_back(bestParticle->idx, &particles,
                                           sysMat[0].attachments.size(),
                                           &(sysMat[0].fixedPoints));

        sysMat[0].controlPointSplines.emplace_back(bestParticle->pos_rest,
                                                   bestParticle->pos_rest, 10,
                                                   bestParticle->idx);
      }
    ...
    case LEFT_RIGHT_CORNERS_2: {
      ...
      std::pair<int, int> p2XY =
          std::make_pair(0, sceneConfig.fabric.gridNumX - 1);
      std::pair<int, int> p3XY =
          std::make_pair(sceneConfig.fabric.gridNumY - 1, 0);
      std::pair<int, int> p4XY = std::make_pair(
          sceneConfig.fabric.gridNumY - 1, sceneConfig.fabric.gridNumX - 1);
      gridXYs = {p1XY, p2XY};
      break;
    }
    ...
    for (std::pair<int, int> gridXY : gridXYs) {
      ...
      sysMat[0].attachments.emplace_back(pIdx, &particles, fixedpointIdx,
                                         &sysMat[0].fixedPoints);
    }
    ...
    case TrajectoryConfigs::CORNERS_2_UP: {
      {
        ...
        Vec3d point00End =
            getInitParticlePos(sceneConfig.fabric.gridNumY - 1, 0);

        Vec3d point0XEnd = getInitParticlePos(sceneConfig.fabric.gridNumY - 1,
                                              sceneConfig.fabric.gridNumX - 1);
        double yUp = 8;

        sysMat[0].controlPointSplines.emplace_back(
            sysMat[0].fixedPoints[0].pos_rest, point00End, yUp, 0);
        ...
        if (sceneConfig.trajectory != CORNERS_1_WEARHAT) {
          ...
          sysMat[0].controlPointSplines.emplace_back(
              sysMat[0].fixedPoints[1].pos_rest, point0XEnd, yUp, 1);
        }
      }
      break;
    }
  ...
  case AttachmentConfigs::CUSTOM_ARRAY: {
    ...
    for (int setIdx = 0; setIdx < sceneConfig.customAttachmentVertexIdx.size();
         setIdx++) {
      std::pair<double, std::vector<int>> &config =
          sceneConfig.customAttachmentVertexIdx[setIdx];
      ...
      for (int idx = 0; idx < config.second.size(); idx++) {
        ...
        sysMat[setIdx].attachments[idx] = AttachmentSpring(
            p.idx, &particles, idx, &(sysMat[setIdx].fixedPoints));
      }
      ...
      for (FixedPoint &p : sysMat[setIdx].fixedPoints) {
        sysMat[setIdx].controlPointSplines.emplace_back(p.pos_rest, p.pos_rest,
                                                        10, p.idx);
      }
    }
    ...
    for (int i = 0; i < particles.size(); i++) {
      for (int j = i + 1; j < particles.size(); j++) {
        ...
        if (posDiff.norm() < a.radii + b.radii) {
          std::printf("WARNING: self-collision at initialization between "
                      "particle %d %d\n",
                      a.idx, b.idx);
        }
      }
...
void Simulation::loadSceneMeshes() {
  arrow1.path =
      std::string(SOURCE_PATH) + "/src/assets/meshes/" + "scene/arrow1.obj";
  arrow1.dim = 3.0;
  arrow1.flipWinding = false;
  arrow2.path =
      std::string(SOURCE_PATH) + "/src/assets/meshes/" + "scene/arrow2.obj";
  arrow2.dim = 0.3;
  ...
  clip.path =
      std::string(SOURCE_PATH) + "/src/assets/meshes/" + "scene/clip.obj";
  clip.dim = 0.8;
  ...
  for (FileMesh *mesh : meshes) {
    ...
    MeshFileHandler::loadOBJFile(mesh->path.c_str(), mesh->points,
                                 mesh->triangles);
    ...
    for (Vec3i &t : mesh->triangles) {
      Vec3d normal;
      if (mesh->flipWinding) {
        normal = Triangle::getNormal(mesh->points[t[2]], mesh->points[t[1]],
                                     mesh->points[t[0]]);
      } else {
        normal = Triangle::getNormal(mesh->points[t[0]], mesh->points[t[1]],
                                     mesh->points[t[2]]);
        ;
      }
...
void Simulation::createClothMesh() {
  ...
  if (sceneConfig.fabric.isModel) {
    std::string path = std::string(SOURCE_PATH) + "/src/assets/meshes/" +
                       sceneConfig.fabric.name;
    modelPoints.clear();
    modelTris.clear();
    ...
    if (sceneConfig.fabric.custominitPos) {
      ...
      if (isTxt)
        modelPoints =
            MeshFileHandler::loadPosFile_txt(sceneConfig.fabric.initPosFile);
      else {
        std::vector<Vec3i> dummy;
        MeshFileHandler::loadOBJFile(sceneConfig.fabric.initPosFile.c_str(),
                                     modelPoints, dummy);
      }
      int newParticleSize = modelPoints.size();
      if (prevParticleSize != newParticleSize) {
        std::printf("WARNING: Custom Mesh Init Pos loading from %s error: mesh "
                    "vertex num is %d but loaded %d vertex\n",
                    sceneConfig.fabric.initPosFile.c_str(), prevParticleSize,
                    newParticleSize);
      } else {
        std::printf("successfully loaded %d vertex init pos from %s\n",
                    newParticleSize, sceneConfig.fabric.initPosFile.c_str());
      }
    }
...
void Simulation::createClothMeshFromConfig() {
  ...
  pointpointConnectionTable.resize(totalParticles,
                                   std::vector<bool>(totalParticles, false));
  ...
  for (int i = 0; i < sceneConfig.fabric.gridNumY; i++) {
    for (int j = 0; j < sceneConfig.fabric.gridNumX; j++) {
      ...
      particles.emplace_back(0.0, mypos, mypos, Vec3d(0, 0, 0), Vec2i(i, j),
                             particles.size());
      particleTriangleMap.emplace_back(std::vector<int>());
      ...
      AssertDebug(gridIndicesToParticle(i, j) ==
                  particles[particles.size() - 1].idx);
    }
  }
...
Simulation *Simulation::createSystem(SceneConfiguration sceneConfig,
  ...
  msSystem->myConfig = {.x0 = msSystem->forwardRecords[0].x,
                        .v0 = msSystem->forwardRecords[0].v,
                        .k_stiff = Triangle::k_stiff};
...
void Simulation::resetParticlesAndPrimitivesToRestPose() {
  ...
  for (Particle &p : particles) {
    ...
    if (std::isnan(p.pos.norm())) {
      std::printf("WARNING: NAN pos(%.2f,%.2f,%.2f) encountered when resetting "
                  "particle %d\n",
                  p.pos[0], p.pos[1], p.pos[2], p.idx);
    }
  }
...
void Simulation::loadWindSim2RealAnimationSequence(
    std::string folderName, std::vector<std::string> files, bool isRyanWhite) {
  ...
  std::vector<std::pair<double, std::vector<int>>> ryanWhiteClipIndices = {
      {0,
       {10, 11, 12, 13, 14, 510, 26, 27, 28, 29, 30, 460, 461, 462, 463, 464,
        495}}};
  std::vector<int> row2 = {496, 497, 459, 458, 457, 456, 31, 32, 24,
                           23,  22,  21,  511, 512, 9,   8,  7,  6};
  ryanWhiteClipIndices[0].second.insert(ryanWhiteClipIndices[0].second.end(),
                                        row2.begin(), row2.end());
  ...
  std::string fullFolderPath =
      std::string(SOURCE_PATH) + "/src/assets/animation/" + folderName + "/";
  ...
  for (int i = 1; i < files.size(); i++) {
    ...
    for (int pIdx = 0; pIdx < particles.size(); pIdx++) {
      x_i.segment(pIdx * 3, 3) = (modelPoints[pIdx] - meshInitTransofrmMinDim) /
                                     meshInitTransofrmScale -
                                 restShapeMaxDim;
    }
    ForwardInformation record = {};
    record.t = sceneConfig.timeStep * i;
    record.x = x_i;
    record.v = (x_i - forwardRecords[forwardRecords.size() - 1].x) /
               sceneConfig.timeStep;
    record.f = empty;
  ...
  if (isRyanWhite) { // interpolate data
    for (int pass = 0; pass < 2; pass++) {
      for (int i = forwardRecords.size() - 1; i >= 1; i--) {
        ...
        newRecord.x_fixedpoints = (forwardRecords[i].x_fixedpoints +
                                   forwardRecords[i - 1].x_fixedpoints) *
                                  0.5;
    ...
    for (int i = 1; i < forwardRecords.size(); i++) {
      forwardRecords[i].v = (forwardRecords[i].x - forwardRecords[i - 1].x) /
                            sceneConfig.timeStep;
  ...
  //      VecXd v_init = (forwardRecords[1].x - forwardRecords[0].x) /
  //      sceneConfig.timeStep;
  //
  //      for (int i = 0; i < particles.size(); i++) {
  ...
  //      }
  std::printf(
      "finished loading animation. forwardRecords now have %zu records\n",
      forwardRecords.size());
...
Simulation::SelfCollisionInformation
Simulation::isSelfCollision(const Triangle &a, const Particle &p3,
                            const Vec3d v_a, const Vec3d v_b) const {
  SelfCollisionInformation info{};
  info.collides = false;
...
double Simulation::calculateLossAndGradient(LossType &lossType,
  ...
  case (MATCH_TRAJECTORY): {
    if (forwardRecords.size() != lossInfo.targetSimulation.size()) {
      std::printf("WARNING: calculate trajectory loss frame number mismatch, "
                  "but records has size %zu while target has size %zu\n",
                  forwardRecords.size(), lossInfo.targetSimulation.size());
    } else {
      //            std::printf("%d ForwardRecordSz: %zu target: %zu calcLoss?:
      //            %d\n",idx,  forwardRecords.size(),
      //            lossInfo.targetSimulation.size(), calculateLoss);
    }

    if (forwardRecords[idx].x.rows() !=
        lossInfo.targetSimulation[idx].x.rows()) {
      std::printf("WARNING: calculate trajectory loss dimension mismatch, but "
                  "records has size %zu while target has size %zu\n",
                  forwardRecords[idx].x.rows(),
                  lossInfo.targetSimulation[idx].x.rows());
    }
    double k_constant = 1.0 / (N * ((int)particles.size()));
    if (calculateLoss) {
      for (int i = 0; i < forwardRecords.size(); i++) {
        L += k_constant * (forwardRecords[i].x - lossInfo.targetSimulation[i].x)
                              .squaredNorm();
        if (std::isnan(L)) {
          std::printf("WARNING: Loss becomes NAN after adding subloss from %d "
                      "in MATCH_TRAJECTORY; record:%.3f target:%.3f\n",
                      i, forwardRecords[i].x.norm(),
                      lossInfo.targetSimulation[i].x.norm());
        }
      }

      dL_dx =
          k_constant * 2 *
          (forwardRecords[lastIdx].x - lossInfo.targetSimulation[lastIdx].x);
    } else {
      dL_dx = k_constant * 2 *
              (forwardRecords[idx].x - lossInfo.targetSimulation[idx].x);
    }
  ...
  case (MATCH_VELOCITY): {
    ...
    if (forwardRecords[idx].v.rows() !=
        lossInfo.targetSimulation[idx].v.rows()) {
      std::printf("WARNING: calculate trajectory loss dimension mismatch, but "
                  "records has size %zu while target has size %zu\n",
                  forwardRecords[idx].v.rows(),
                  lossInfo.targetSimulation[idx].v.rows());
    }
    double k_constant = 1.0 / (N * ((int)particles.size()));
    if (calculateLoss) {
      for (int i = 0; i < forwardRecords.size(); i++) {
        L += k_constant * (forwardRecords[i].v - lossInfo.targetSimulation[i].v)
                              .squaredNorm();
        if (std::isnan(L)) {
          std::printf("WARNING: Loss becomes NAN after adding subloss from %d "
                      "in MATCH_TRAJECTORY; record:%.3f target:%.3f\n",
                      i, forwardRecords[i].v.norm(),
                      lossInfo.targetSimulation[i].v.norm());
        }
      }

      dL_dv =
          k_constant * 2 *
          (forwardRecords[lastIdx].v - lossInfo.targetSimulation[lastIdx].v);
    } else {
      dL_dv = k_constant * 2 *
              (forwardRecords[idx].v - lossInfo.targetSimulation[idx].v);
    }
  ...
  case (MATCH_TRAJECTORY_MAX): {
    ...
    for (int i = 0; i < forwardRecords.size(); i++) {
      int lossFrame =
          k_constant *
          (forwardRecords[i].x - lossInfo.targetSimulation[i].x).squaredNorm();
      if (lossFrame > L) {
        L = lossFrame;
        maxFrameId = i;
      }
  ...
  case (DRESS_ANGLE): {
    ...
    double targetHeight =
        restShapeMinDim[1] +
        (restShapeMaxDim[1] - restShapeMinDim[1]) * lossInfo.targetTwirlHeight;
  ...
  case (MATCHSHAPE_TRANSLATION_INVARINT): {
    ...
    for (int i = 0; i < particles.size(); i++) {
      ...
      curShapeTranslationFree.segment(i * 3, 3) -=
          curShapeTranslationFree.segment(0, 3);
    }

    int numParticles = particles.size();
    L += 1.0 / numParticles *
         (curShapeTranslationFree - targetShape).squaredNorm();
  ...
  case (ASSISTED_DRESSING_KEYPOINTS): {
    int totalKeyPoints = lossInfo.targetPosPairs.size();
    {
      ...
      for (CorresPondenceTargetInfo &pIdxTargetLocPair :
           lossInfo.targetPosPairs) {
...
void Simulation::resetSystemWithParams(
    Simulation::BackwardTaskInformation &taskConfiguration,
  ...
  if (taskConfiguration.dL_dcontrolPoints) {
    for (int sysMatId = 0; sysMatId < sysMat.size(); sysMatId++) {
      sysMat[sysMatId].controlPointSplines =
          param.controlPointSplines[sysMatId];
    }
  }
  ...
  if (std::isnan(forwardRecords[0].x.norm()) ||
    ...
    std::printf("WARNING: NAN encountered after reset: x: %.4f v: %.4f\n",
                forwardRecords[0].x.norm(), forwardRecords[0].v.norm());
  }
...
void Simulation::calculateFiniteDiffLossArr(
    Simulation::ParamInfo paramPlus, Simulation::ParamInfo paramMinus,
    double delta, std::vector<ForwardInformation> &centerForward,
  ...
  std::vector<double> L_plus(FORWARD_STEPS + 1, 0),
      L_minus(FORWARD_STEPS + 1, 0);
  for (int startSepIdx = 0; startSepIdx < FORWARD_STEPS; startSepIdx++) {
    L_arr[startSepIdx] = 0;
  }
  resetSystemWithParams(taskConfiguration, paramPlus);
  int idxSpace =
      200; // since full backward records require FORWARD_STEPS^2 steps
           // simulation, we calculate records with spacing instead.
  int startSepIdx = 0;
  ...
  while (startSepIdx < FORWARD_STEPS) {
    ...
    setParticlePosVelToVec(std::make_pair(forwardRecords[startSepIdx].x,
                                          forwardRecords[startSepIdx].v));

    for (int id = startSepIdx; id < FORWARD_STEPS; id++)
      step();
    L_plus[startSepIdx] = (calculateLossAndGradient(
        lossType, lossInfo, dL_dlastx, dL_dlastv, 0, true));
  ...
  while (startSepIdx < FORWARD_STEPS) {
    ...
    for (int id = startSepIdx; id < FORWARD_STEPS; id++)
      step();
    L_minus[startSepIdx] = calculateLossAndGradient(
        lossType, lossInfo, dL_dlastx, dL_dlastv, 0, true);
  ...
  for (int startSepIdx = 0; startSepIdx < FORWARD_STEPS; startSepIdx++) {
    L_arr[startSepIdx] =
        (L_plus[startSepIdx] - L_minus[startSepIdx]) / (2 * delta);
  }
}

std::vector<Simulation::BackwardInformation>
Simulation::finiteDifferenceBackward(
    Simulation::BackwardTaskInformation taskConfiguration, LossType lossType,
    Simulation::LossInfo &lossInfo, int FORWARD_STEPS,
    Simulation::ParamInfo guess,
    const std::function<void(const std::string &)> &setTextBoxCB) {
  ...
  for (int constraintTypeIdx = 0;
       constraintTypeIdx < Constraint::CONSTRAINT_NUM; constraintTypeIdx++) {
    ...
    if (taskConfiguration.dL_dk_pertype[constraintTypeIdx]) {
      ...
      calculateFiniteDiffLossArr(paramPlus, paramMinus, delta, centerForward, L,
                                 taskConfiguration, lossType, lossInfo,
  ...
  if (taskConfiguration.dL_dcontrolPoints) {
    for (int setidx = guess.controlPointSplines.size() - 1;
         setidx < guess.controlPointSplines.size(); setidx++) {
      ...
      for (int splineIdx = 0; splineIdx < spline_center.size(); splineIdx++) {
        ...
        std::printf("spline %d has parameter number %d\n", splineIdx,
                    splineParameterNumber);
        for (int dim = 0; dim < splineParameterNumber; dim++) {
          setTextBoxCB("fd dL_dspline-" + std::to_string(splineIdx) + "dim-" +
                       std::to_string(dim) + "\n");
          paramPlus = paramMinus = guess;
          ...
          paramPlus.controlPointSplines[setidx][splineIdx].updateControlPoints(
              deltaStep);
          paramMinus.controlPointSplines[setidx][splineIdx].updateControlPoints(
              -deltaStep);

          calculateFiniteDiffLossArr(
              paramPlus, paramMinus, delta, centerForward, L, taskConfiguration,
              lossType, lossInfo, FORWARD_STEPS, setTextBoxCB);

          int paramIdxOffset = Spline::getParameterNumberBeforeIdx(
              sysMat[0].controlPointSplines, splineIdx);
...
void Simulation::exportCurrentSimulation(std::string fileName) {
  ...
  for (int i = 0; i < forwardRecords.size(); i++) {
    ...
    areaTotalStr +=
        "Frame " + std::to_string(i) + ":" + d2str(areaTotal, 7) + "\n";
    ...
    for (int fixedPointId = 0;
         fixedPointId < forwardRecords[i].x_fixedpoints.rows() / 3;
         fixedPointId++) {
      Vec3d clipCenter =
          forwardRecords[i].x_fixedpoints.segment(fixedPointId * 3, 3);
...
std::vector<Simulation::BackwardInformation> Simulation::runBackwardTask(
    Simulation::BackwardTaskInformation taskConfiguration, LossType lossType,
    Simulation::LossInfo &lossInfo,
    Simulation::TaskSolveStatistics &taskStatistics, int FORWARD_STEPS,
  ...
  Simulation::forwardConvergenceThreshold =
      taskConfiguration.forwardAccuracyLevel;
  Simulation::backwardConvergenceThreshold =
      taskConfiguration.backwardAccuracyLevel;
  ...
  double L = calculateLossAndGradient(lossType, lossInfo, dL_dlastx, dL_dlastv,
                                      N - 1, true);
  ...
  taskStatistics.completeForwardLog.emplace_back(
      guess, forwardRecords[forwardRecords.size() - 1]);
  calcualteSeperateAt_p = false;
  ...
  if ((FORWARD_STEPS + 1) != forwardRecords.size()) {
    std::printf(
        "WARNING: invariant violated: FORWARD_STEPS:%d fowardRecords: %zu\n",
        FORWARD_STEPS,
  ...
  for (int idx = FORWARD_STEPS; idx >= 1; idx--) {
    ...
    ForwardInformation &record =
        forwardRecords[idx]; // foward steps from [x,v]_{idx-1} --> [x,v]_{idx}
    if (idx % 20 == 0) {
      std::printf("%d..", idx);
      std::fflush(stdout);
    }
  ...
  fullBackwardRecords[0].correspondingForwardIdxInStats =
      taskStatistics.totalForwardSim - 1;

  taskStatistics.totalBackprop++;
  taskStatistics.completeBackwardLog.emplace_back(guess,
                                                  fullBackwardRecords[0]);
  return fullBackwardRecords;
}

void Simulation::exportOptimizationRecords(Demos demoIdx,
  ...
  writeStringToFile(OUTPUT_PARENT_FOLDER + experimentName +
                        "/groundtruth/param.txt",
  ...
  for (int i = 0; i < backwardOptimizationRecords.size(); i++) {
    ...
    writeStringToFile(iterationFolder + "/gradientInfo.txt",
                      backwrdInfoAndGradToString(
                          taskInfo, backwardOptimizationRecords[i].second[0]));
    writeStringToFile(
        iterationFolder + "/forwardInfo.txt",
        forwardInfoToString(
            taskInfo,
            backwardOptimizationRecords[i]
                .first[backwardOptimizationRecords[i].first.size() - 1]));
  }
}

void Simulation::exportStatistics(Demos demoIdx,
                                  Simulation::TaskSolveStatistics &statistics,
                                  Simulation::BackwardTaskInformation taskInfo,
                                  bool writePerf) {
  if (!statistics.configWritten) {
    std::string experimentName =
        sceneConfig.name + "-randseed-" + std::to_string(taskInfo.srandSeed) +
    ...
    Logging::logMagenta("Optimization progress is saved to:" +
  ...
  for (int i = statistics.backwardWritten; i < currentBackwardInfoSize; i++) {
    BackwardInformation &backwardInfo =
        statistics.completeBackwardLog[i].second;
  ...
  if (writePerf) {
    ...
    out += "BackwardSolver:" +
           boolToString("Jacobi", "Direct", backwardGradientForceDirectSolver) +
           "\n";
    ...
    for (int i = 0; i < statistics.completeBackwardLog.size(); i++) {
      ...
      out += "Jacobi total converged:" + std::to_string(info.convergedAccum) +
             "\n";
      out +=
          "Total backward props:" + std::to_string(sceneConfig.stepNum) + "\n";
    }
    ...
    for (int i = 0; i < statistics.completeForwardLog.size(); i++) {
      ...
      out += "iter" + std::to_string(i) + ":" +
             d2str(info.totalRuntime / 1000000.0, 8) + "\n";
    }
    ...
    out +=
        "Total Forward Time:" + d2str(totalForwardTime / 1000000.0, 6) + "\n";
    out +=
        "Total Backward Time:" + d2str(totalBackwardTime / 1000000.0, 6) + "\n";
    out += "Total Time:" +
           d2str((totalForwardTime + totalBackwardTime) / 1000000.0, 6) + "\n";
    writeStringToFile(OUTPUT_PARENT_FOLDER + statistics.experimentName +
                          subFolder + "perf.txt",
                      out);
  }
}

void Simulation::exportConfig(int demoIdx,
  ...
  myfile << "demoIdx:" << demoIdx << " "
         << "\n";
  myfile << "SceneBbox: min:" << vec2str(config.sceneBbox.min, 4)
         << " max:" << vec2str(config.sceneBbox.max, 4) + "\n";
  ...
  myfile << "Object Number:"
         << primitives.size() + 1 + (forwardRecords[0].x_fixedpoints.rows() / 3)
         << "\n";
  std::vector<std::pair<std::string, Vec3d>> objNameAndColor;
  objNameAndColor.emplace_back("0-CLOTH", config.fabric.color);
  for (int primId = 0; primId < primitives.size(); primId++) {
    Primitive *prim = primitives[primId];
    objNameAndColor.emplace_back(
        std::to_string(primId + 1) + "-" +
            Primitive::primitiveTypeStrings[prim->type],
        prim->color);
  }

  for (int fixedPointId = 0;
       fixedPointId < forwardRecords[0].x_fixedpoints.rows() / 3;
       fixedPointId++) {
    objNameAndColor.emplace_back(
        std::to_string(fixedPointId + 1 + primitives.size()) + "-CLIP",
        COLOR_WOOD);
  }
...
void Simulation::exportStatsSimulations(
  ...
  for (int i = prevWrriten; i < newWritten; i++) {
    MeshFileHandler::saveOBJFile(subFolder + "0-CLOTH_forwardlastframe_iter_" +
                                     std::to_string(i),
...
void Simulation::exportSimulation(std::string fileName,
  ...
  for (int i = 0; i < records.size(); i++) {
    ...
    if (exportFixedPointMesh)
      for (int fixedPointId = 0;
           fixedPointId < records[i].x_fixedpoints.rows() / 3; fixedPointId++) {
        Vec3d clipCenter =
            records[i].x_fixedpoints.segment(fixedPointId * 3, 3);
        std::vector<Vec3i> primMesh;
        VecXd primPos;
        clip.getMesh(primMesh, primPos, clipCenter);
        MeshFileHandler::saveOBJFile(
...
std::string Simulation::taskInfoToString(
  ...
  out +=
      "Backward Accuracy:" + d2str(taskInfo.backwardAccuracyLevel, 11) + "\n";
  ...
  for (int i = 0; i < Constraint::CONSTRAINT_NUM; i++)
    out += boolToStringTrueOnly(CONSTRAINT_TYPE_STRINGS[i] + ": ON",
    ...
    for (int i = 0; i < taskInfo.splineTypes.size(); i++) {
      if (!taskInfo.splineTypes.empty()) {
        ...
        for (int j = 0; j < taskInfo.splineTypes[i].size(); j++) {
          out += "s_" + std::to_string(i) + ":" +
                 SPLINE_TYPE_STRINGS[taskInfo.splineTypes[i][j]] + "\n";
        }
      }
    }
  }
...
std::string Simulation::parameterToString(
    const Simulation::BackwardTaskInformation &taskInfo,
    Simulation::ParamInfo paramGuess) {
  std::string out;
  out += "============Parameter Info:======================\n";

  for (int i = 0; i < Constraint::ConstraintType::CONSTRAINT_NUM; i++) {
    if (taskInfo.dL_dk_pertype[i]) {
      out += "k_" + CONSTRAINT_TYPE_STRINGS[i] + ":" +
             d2str(paramGuess.k_pertype[i], 6) + "\n";
    }
  }
  ...
  if (taskInfo.dL_dmu) {
    for (std::pair<int, double> pair : paramGuess.mu) {
      out += "mu from prim" + std::to_string(pair.first) + ":" +
             d2str(pair.second, 6) + "\n";
    }
  }
  ...
  if (taskInfo.dL_dx0) {
    ...
    for (int i = 0; i < paramGuess.x0.rows() / 3; i += 200) {
      out += "pId-" + std::to_string(i) +
             vecXd2str(paramGuess.x0.segment(i * 3, 3), 5) + "\n";
    }
  }
  ...
  if (taskInfo.dL_dcontrolPoints) {
    for (int matSetIdx = 0; matSetIdx < paramGuess.controlPointSplines.size();
         matSetIdx++) {
      out += "====set" + std::to_string(matSetIdx) + "=====\n";
      for (int i = 0; i < paramGuess.controlPointSplines[matSetIdx].size();
           i++) {
        Spline &spline = paramGuess.controlPointSplines[matSetIdx][i];
        for (Spline::Segment &s : spline.segments) {
          out +=
              "t[" + d2str(s.startFraction) + "," + d2str(s.endFraction) + "]:";
...
std::string Simulation::backwrdInfoAndGradToString(
    const Simulation::BackwardTaskInformation &taskInfo,
  ...
  out +=
      "Number of Converged Iter:" + std::to_string(grad.convergedAccum) + "\n";
  out +=
      "Total Backward Iter:" + std::to_string(grad.backwardTotalIters) + "\n";
  out += "Backward Total Runtime[ms]:" + d2str(grad.totalRuntime / 1000.0, 7) +
         "\n";
  out +=
      "Backward Non Solve Time[ms]:" +
      std::to_string(grad.accumSolvePerformanceReport.nonSolveTimeMicroseconds /
                     1000.0) +
      "\n";
  out +=
      "Backward Direct-Solve Time[ms]:" +
      std::to_string(grad.accumSolvePerformanceReport.solveDirectMicroseconds /
                     1000.0) +
      "\n";
  out += "Backward Iter-Solve Time[ms]:" +
         std::to_string(
             grad.accumSolvePerformanceReport.solveIterativeMicroseconds /
             1000.0) +
         "\n";
  ...
  for (TimerEntry &breakdown : grad.accumTimer) {
    ...
    out += label + "[ms]:" + d2str(duration / 1000.0, 5) + "\t\t|" +
           d2str(percentage * 100, 3) + "%\n";
  }
  double percentageDirect =
      grad.accumSolvePerformanceReport.solveDirectMicroseconds * 1.0 /
      grad.totalRuntime;
  double percentageIter =
      grad.accumSolvePerformanceReport.solveIterativeMicroseconds * 1.0 /
      grad.totalRuntime;
  out +=
      "Direct-Solve[ms]:" +
      d2str(grad.accumSolvePerformanceReport.solveDirectMicroseconds / 1000.0,
            5) +
      "\t|" + d2str(percentageDirect * 100, 3) + "%\n";
  out += "Iter-Solve[ms]:" +
         d2str(grad.accumSolvePerformanceReport.solveIterativeMicroseconds /
                   1000.0,
               5) +
  ...
  for (int i = 0; i < Constraint::ConstraintType::CONSTRAINT_NUM; i++) {
    if (taskInfo.dL_dk_pertype[i]) {
      out += "dL/dk_" + CONSTRAINT_TYPE_STRINGS[i] + ":" +
             d2str(grad.dL_dk_pertype[i], 5) + "\n";
    }
  }
  ...
  if (taskInfo.dL_dmu) {
    for (std::pair<int, double> pair : grad.dL_dmu) {
      out += "dL_dmu_" + std::to_string(pair.first) + ":" +
             d2str(pair.second, 5) + "\n";
    }
  }
  ...
  if (taskInfo.dL_dx0) {
    ...
    for (int i = 0; i < grad.dL_dx.rows() / 3; i += 200) {
      out += "pId-" + std::to_string(i) +
             vecXd2str(grad.dL_dx.segment(i * 3, 3), 5) + "\n";
    }
  }
...
VecXd Simulation::getParticleNormals(std::vector<Triangle> mesh,
                                     const VecXd &x_now) {
