...
public:
  enum CollisionType { TAKE_OFF, STICK, SLIDE, NUM_CASES };
  ...
  Simulation(Vec3d center)
      : systemCenter(center),
        ...
        plane1(Vec3d(0, -0.01, 5), Vec3d(-10, -0.01, -4), Vec3d(10, -0.01, -4),
               COLOR_GRAY50),
  ...
  void rotatePointsAccordingToConfig(std::vector<Vec3d> &particleIn) {
    ...
    case Orientation::BACK: {
      std::printf("orientation down: rotate\n");
      particleIn = rotatePointsAroundCenter(
          particleIn, axisToRotation(Vec3d(0, 0, 1), Vec3d(1, 0, 0)) *
