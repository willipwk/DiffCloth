...
public:
  ...
  nanogui::Button *forwardStartSimulationButton = nullptr,
                  *playbackSimulationButton = nullptr,
                  *forwardStopSimulationButton = nullptr,
                  *backwardPlayGroundtruthSimulationButton = nullptr,
                  *backwardPlayRecordButton = nullptr;
  nanogui::TextBox *simulationInfoTextBox = nullptr, *infoTextBox = nullptr,
                   *backwardInfo1 = nullptr, *backwardInfo2 = nullptr;
  ...
  nanogui::detail::FormWidget<int> *timeStepSetter, *dimControl,
      *precisionSetter;
  nanogui::detail::FormWidget<double> *strechingStiffnessControl,
      *bendingStiffnessControl, *densityControl;
  nanogui::ref<nanogui::Window> mainConfigWindow, forwardSimControlWindow,
      reinitWindow, backwardSimControlWindow, backwardSimInfoWindow,
      forwardInfoWindow, backwardSimFiniteDiffInfoWindow, particleInfoWindow;
  ...
  bool useFiniteDiff = false, useLBFGS = false, randomInit = true,
       perfTest = false, directBackward = false;
  ...
  SliderTextBoxWidgets backwardRecordSliderText, backwardFrameSliderText,
      forwardFrameSliderText;
  ...
  BackwardGradientInfoWidgets gradientInfoWidgets = {},
                              finiteDiffInfoWidgets = {};
  ...
  bool isSimulationOn = false, isplaybackSimulationOn = false, isforward = true,
       backwardNeedsInitiation = false, backwardRunning = false,
       playGroundtruth = false, playLineSearch = false,
       renderWithGroundTruth = false;
  int selectedDemoIdx = 0;
  int playBackId = 0, backwardSelectedRecordId = -1, particleId = 0,
      triangleId = 0;
  int meshDim = 7;
  double strechingStiffness = 50.0, bendingStiffness = 0.0,
         wireframeThickness = 0.07;
  ...
  nanogui::Color color_background =
      nanogui::Color(0.2022f, 0.2022f, 0.2022f, 1.f);
  nanogui::Color red = nanogui::Color(1.0f, 0.0f, 0.0f, 1.f);

  SceneConfigArray currentSceneConfig;
  nanogui::Widget *panel;
  std::vector<std::string> renderModeStrings = std::vector<std::string>{
      "FABRIC_COLOR", "SOLID_CUSTOM", "WIREFRAME",  "WIREFRAME_COLORED",
      "SOLID-2SIDES", "DEFORMATION",  "NORMAL_MAP", "RANDOM_COLOR"};
  enum RenderMode {
    FABRIC_COLOR,
    SOLID_CUSTOM,
    WIREFRAME,
  ...
  void updateProjectionMatrix() {
    projectionMat =
        glm::perspective(glm::radians(dof), width * 1.0f / height, zNear, zFar);
  }

  Viewer(bool isSimpleViewer = false)
      : width(1600), height(1200), lastX(400), lastY(300), zNear(0.1),
        zFar(200), firstMouse(true), dof(60.0), isSimpleViewer(isSimpleViewer) {
    camera = Camera();
  ...
  void
  gradientInfoWindowContentUpdate(BackwardGradientInfoWidgets &gradientWidgets,
                                  Simulation::BackwardInformation &gradient,
                                  bool isFiniteDiff);

  void
  forwardInfoWindowContentUpdate(ForwardInfoWidgets &forwardWidgets,
                                 Simulation::ForwardInformation &forwardInfo);
  ...
  void reinitWithSceneFabric() {
    Simulation::SceneConfiguration config =
        OptimizationTaskConfigurations::sceneConfigArrays[currentSceneConfig];
    ...
    camera.setCamPos(glm::vec3(simSystems[0]->sceneConfig.camPos[0],
                               simSystems[0]->sceneConfig.camPos[1],
  ...
  nanogui::Vector2i modelToScreen(Vec3d &pose) {
    ...
    res = nanogui::Vector2i((int)((x * 0.5 + 0.5) * width),
...
private:
  ...
  void addBackwardSimulationInfoWidets(
  ...
  void addForwardSimulationInfoWidets(
