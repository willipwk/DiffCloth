...
VertexArrayObject BufferData::drawLightSetupVAO() {
  VertexArrayObject lightVAO = VertexArrayObject(
      (float *)BufferData::cubeVertices, 36, 8 * sizeof(float));
  lightVAO.addVertexAttrib(0, 3, 0);
  return lightVAO;
}

void BufferData::drawRectangle(VertexArrayObject &vao, IndexArrayObject &iao,
                               Shader shader) {
  shader.use();
  vao.use();
  iao.use();
...
VertexArrayObject BufferData::drawTriangleSetup(glm::vec3 a, glm::vec3 b,
                                                glm::vec3 c) {
  float vertices[] = {
...
VertexArrayObject BufferData::drawcubeSetupVAO() {
  VertexArrayObject VAO = VertexArrayObject((float *)BufferData::cubeVertices,
                                            36, 8 * sizeof(float));
  VAO.addVertexAttrib(0, 3, 0);
...
void BufferData::drawTriangle(Shader triangleShader,
