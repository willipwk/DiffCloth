...
GLFWwindow *Viewer::init() {
  ...
  windowName.append(std::to_string(width))
      .append(" ")
  ...
  glfwSetCursorPosCallback(glfWwindow, [](GLFWwindow *window, double x,
                                          double y) {
...
void Viewer::mouse_callback(double xpos, double ypos) {
  ...
  float yoffset =
      lastY -
      (float)ypos; // reversed since y-coordinates range from bottom to top
  lastX = (float)xpos;
  lastY = (float)ypos;
...
#ifdef USING_MACBOOK
  // TODO: I don't know why on my mac using width and height just renders to
...
void Viewer::processInput() {
  ...
  if (glfwGetKey(glfWwindow, GLFW_KEY_RIGHT) == GLFW_PRESS) {
    ...
    glm::vec3 move = camera.getCamPos() +
                     glm::vec3(tangent[0], tangent[1], tangent[2]) * 0.5f;
...
std::vector<nanogui::Vector2i>
Viewer::modelToScreen(std::vector<Vec3d> &poses) {
  ...
  for (const Vec3d &s : poses) {
    ...
    res.emplace_back(nanogui::Vector2i((int)((x * 0.5 + 0.5) * width),
                                       (int)(((-y) * 0.5 + 0.5) * height)));
  }
...
void Viewer::updateOnScreenInfos() {
  ...
  if (isforward && (!isSimpleViewer)) {
    simulationInfoTextBox->setValue(
        "Total Timestep: " +
        std::to_string(simSystems[0]->forwardRecords.size()));
  }
}
...
void Viewer::resetAllSimulation() {
  ...
  forwardFrameSliderText.slider->callback()(
      forwardFrameSliderText.slider->value());
}
...
void Viewer::setupNanogui() {
  ...
  if (isSimpleViewer) {
    ...
    simulationInfoTextBox->setValue(
        "Here is a textbox for use blahblahblahblahblahblah");
    ...
    for (Simulation *sim : simSystems) {
      symPosCenters.emplace_back(sim->systemCenter + sim->sphere5.center +
                                 Vec3d(0, 21, 0));
    }
    ...
    backwardInfo.dL_dsplines = finitediffGradientInfo.dL_dsplines =
        simSystems[0]->backwardInfoDefault.dL_dsplines;
    addForwardSimulationInfoWidets("Forward Info", forwardInfoWidgets,
                                   forwardInfo, forwardInfoWindow, 300);
    addBackwardSimulationInfoWidets("Backward Gradient Log",
                                    gradientInfoWidgets, backwardInfo,
                                    backwardSimInfoWindow, 500, false);
    addBackwardSimulationInfoWidets(
        "Finite Difference Gradient", finiteDiffInfoWidgets,
        finitediffGradientInfo, backwardSimFiniteDiffInfoWindow, 900, true);
    ...
    glfwSetCharCallback(glfWwindow, [](GLFWwindow *window,
    ...
    glfwSetScrollCallback(glfWwindow, [](GLFWwindow *window, double x,
                                         double y) {
      Viewer *viewer = static_cast<Viewer *>(glfwGetWindowUserPointer(window));
...
void idxToFloatAndText(int idx, int totalSize, double &floatVal,
                       std::string &text) {
  // idx:[0,totalSize-1]
  // floatVal: [0,1]
...
template <typename T>
void formWidgetSetSpinnableWithIncrement(nanogui::detail::FormWidget<T> *widget,
                                         T inc) {
...
void Viewer::addSceneControlWidgets() {
  mainConfigWindow =
      SettingsControl->addWindow(Eigen::Vector2i(30, 0), "Cloth Simulation");
  ...
  if (simSystems.size() > 0) {
    ...
    for (int idx = 0; idx < prims.size(); idx++) {
      ...
      SettingsControl->addWidget(p->primitiveTypeStrings[p->type].c_str(),
                                 panel);
      ...
      SettingsControl->addVariable<bool>(
          ...
          [=]() -> bool { return simSystems[0]->primitives[idx]->isEnabled; },
          panel);
      ...
      nanogui::detail::FormWidget<float> *muControl =
          SettingsControl->addVariable<float>(
              "mu",
              [=](const float &v) {
                for (int i = 0; i < simSystems.size(); i++) {
              ...
              [=]() -> float { return simSystems[0]->primitives[idx]->mu; },
              panel);
  ...
  nanogui::Widget *panel = new nanogui::Widget(mainConfigWindow);
  panel->setLayout(new nanogui::GridLayout(nanogui::Orientation::Horizontal, 6,
  ...
  SettingsControl->addVariable<bool>(
      "wind", [&](const bool &v) { Simulation::windEnabled = v; },
  ...
  SettingsControl->addVariable<bool>(
      "collision", [&](const bool &v) { visualizeCollision = v; },
      [&]() -> bool { return visualizeCollision; }, panel);

  SettingsControl->addVariable<bool>(
      "grid ", [&](const bool &v) { renderGrid = v; },
      [&]() -> bool { return renderGrid; }, panel);

  SettingsControl->addVariable<bool>(
      "AABB ", [&](const bool &v) { renderAABB = v; },
      [&]() -> bool { return renderAABB; }, panel);

  SettingsControl->addVariable<bool>(
      "spline", [&](const bool &v) { splineVisualization = v; },
      [&]() -> bool { return splineVisualization; }, panel);

  SettingsControl->addVariable<bool>(
      "axis", [&](const bool &v) { renderAxis = v; },
      [&]() -> bool { return renderAxis; }, panel);

  SettingsControl->addVariable<bool>(
      "lights", [&](const bool &v) { renderLights = v; },
      [&]() -> bool { return renderLights; }, panel);

  SettingsControl->addVariable<bool>(
      "pairs", [&](const bool &v) { renderPosPairs = v; },
      [&]() -> bool { return renderPosPairs; }, panel);

  SettingsControl->addVariable<bool>(
      "self-collisionLayer",
      [&](const bool &v) { visualizeSelfCollisionLayers = v; },
      [&]() -> bool { return visualizeSelfCollisionLayers; }, panel);
  SettingsControl->addVariable<bool>(
      "collisionNormal", [&](const bool &v) { collisionShowNormal = v; },
      [&]() -> bool { return collisionShowNormal; }, panel);

  nanogui::detail::FormWidget<int> *layerIdWidget =
      SettingsControl->addVariable<int>(
          "layerId", [&](const int &v) { selfCollisionLayerVisId = v; },
          [&]() -> int { return selfCollisionLayerVisId; }, panel);
  layerIdWidget->setMinValue(-1);
  formWidgetSetSpinnableWithIncrement(layerIdWidget, 1);

  SettingsControl->addVariable("Render Mode", myRendermode, true)
      ->setItems(renderModeStrings);
  SettingsControl
      ->addVariable("WindConfig", simSystems[0]->sceneConfig.windConfig, true)
      ->setItems(windConfigStrings);

  panel = new nanogui::Widget(mainConfigWindow);
  panel->setLayout(new nanogui::GridLayout(nanogui::Orientation::Horizontal, 4,
                                           nanogui::Alignment::Middle, 0, 0));
  ...
  timeStepSetter = SettingsControl->addVariable<int>(
      ...
      [&]() -> int { return (int)(1.0 / simSystems[0]->sceneConfig.timeStep); },
      panel);
  ...
  precisionSetter = SettingsControl->addVariable<int>(
      "forward_thresh",
      [&](const int &v) {
        Simulation::forwardConvergenceThreshold = std::pow(10, -v);
      },
      [&]() -> int {
        return (int)(-std::log10(Simulation::forwardConvergenceThreshold));
      },
      panel);
  ...
  SettingsControl->addVariable("custom color", color_custom)
      ->setFinalCallback([](const nanogui::Color &c) {
        std::cout << "color_custom: [" << c.r() << ", " << c.g() << ", "
                  << c.b() << ", " << c.w() << "]" << std::endl;
      });

  SettingsControl->addVariable("color_2sides_1", color_doubleside_1)
      ->setFinalCallback([](const nanogui::Color &c) {
        std::cout << "color_doubleside_1: [" << c.r() << ", " << c.g() << ", "
                  << c.b() << ", " << c.w() << "]" << std::endl;
      });

  SettingsControl->addVariable("color_2sides_2", color_doubleside_2)
      ->setFinalCallback([](const nanogui::Color &c) {
        std::cout << "color_doubleside_2: [" << c.r() << ", " << c.g() << ", "
                  << c.b() << ", " << c.w() << "]" << std::endl;
      });

  SettingsControl->addVariable("color_background", color_background)
      ->setFinalCallback([](const nanogui::Color &c) {
        std::cout << "color_background: [" << c.r() << ", " << c.g() << ", "
                  << c.b() << ", " << c.w() << "]" << std::endl;
      });
  nanogui::detail::FormWidget<float> *wireframe =
      SettingsControl->addVariable<float>(
          "wireframe thickness",
          [&](const float &v) { wireframeThickness = v; },
          [&]() -> float { return wireframeThickness; });
  formWidgetSetSpinnableWithIncrement(wireframe, 0.01f);
  nanogui::detail::FormWidget<float> *windControl =
      SettingsControl->addVariable<float>(
          "wind size", [&](const float &v) { Simulation::windNorm = v; },
          [&]() -> float { return Simulation::windNorm; });
...
void Viewer::addCameraControlWidgets() {
  ...
  panel->setLayout(
      new BoxLayout(nanogui::Orientation::Horizontal, Alignment::Middle, 0, 0));
  SettingsControl->addWidget("CameraPos", panel);
  for (int i = 0; i < 3; i++) {

    nanogui::detail::FormWidget<float> *widget =
        SettingsControl->addVariable<float>(
            "",
            [=](const float &v) {
  ...
  SettingsControl->addButton("Set to SceneConfig", [&]() {
    ...
    camera.setLookAt(
        Simulation::getLookAtPos(simSystems[0], simSystems[0]->sceneConfig));
  });
}
  ...
  if (isplaybackSimulationOn) {
    ...
    if (backwardRecordNum == 0) {
      ...
      if (backwardSelectedRecordId != -1) {
        std::string text;
        double val;
        idxToFloatAndText(backwardSelectedRecordId, backwardRecordNum, val,
                          text);
    ...
    if (simSystems[0]->groundTruthForwardRecords.size() ==
        simSystems[0]->sceneConfig.stepNum + 1) {
...
void Viewer::checkBackwardPlaybackFinishedCallback() {
  bool canRender = (playGroundtruth) || (playLineSearch) ||
                   ((!playGroundtruth) && (!playLineSearch) &&
  ...
  int totalSize = simSystems[0]
                      ->backwardOptimizationRecords[backwardSelectedRecordId]
                      .first.size();
  if (playGroundtruth) {
  ...
  if (isplaybackSimulationOn) { // play and advance
    if (playBackId < totalSize - 1) {
      ...
      backwardFrameSliderText.textbox->setValue(
          std::to_string(playBackId + 1) + "/" + std::to_string(totalSize));
      backwardFrameSliderText.slider->setValue(
          totalSize <= 1 ? 0.0 : playBackId / (totalSize - 1.0));
    } else {
...
void Viewer::addForwardSimulationInfoWidets(
    std::string title, ForwardInfoWidgets &infoWidgets,
    Simulation::ForwardInformation &forwardInfo,
    nanogui::ref<nanogui::Window> window, int widthOffset) {
  using namespace nanogui;
  window = SettingsControl->addWindow(
      Eigen::Vector2i(width - widthOffset, height - 500), title);
  forwardInfo = {};
  infoWidgets.convergeIter =
      SettingsControl->addVariable("converge iter", forwardInfo.convergeIter);
  infoWidgets.accumulateIter =
      SettingsControl->addVariable("accumulate iter", forwardInfo.cumulateIter);

  infoWidgets.selfCollisionNumber =
      SettingsControl->addVariable("self-collisions", forwardInfo.convergeIter);
  infoWidgets.externalCollisionNumber = SettingsControl->addVariable(
      "external-collisions", forwardInfo.convergeIter);
  infoWidgets.avgDeformation = SettingsControl->addVariable(
      "avgDeformation", forwardInfo.avgDeformation);
  ...
  infoWidgets.converged =
      SettingsControl->addVariable("converged", forwardInfo.converged);

  panel = new Widget(window);
  panel->setLayout(
      new BoxLayout(nanogui::Orientation::Horizontal, Alignment::Fill, 0, 0));
  infoWidgets.totalTime = SettingsControl->addVariable(
      "totalRuntime[s]", forwardInfo.avgDeformation);
  infoWidgets.iterTime = SettingsControl->addVariable(
      "iterRuntime[s]", forwardInfo.avgDeformation);
  ...
  infoWidgets.timerReportIdxControl = SettingsControl->addVariable<int>(
      ...
      [&](const int &v) {
        if (v !=
            std::clamp(v, 0,
                       (int)forwardInfo.timer.timeMicroseconds.size() - 1)) {
          return;
        }
        ...
        infoWidgets.accumTimerReport->setValue(
            label + ":" + accumDurationStr + "|" +
            std::to_string(percentageAccum) + "%");
      },
...
void Viewer::forwardInfoWindowContentUpdate(
    ForwardInfoWidgets &forwardWidgets,
    Simulation::ForwardInformation &forwardInfo) {
  ...
  forwardWidgets.externalCollisionNumber->setValue(
      forwardInfo.collisionInfos.first.first.size());
  forwardWidgets.selfCollisionNumber->setValue(
      forwardInfo.collisionInfos.first.second.size());
  forwardWidgets.avgDeformation->setValue(
      roundToDigits(forwardInfo.avgDeformation, 3));
  forwardWidgets.timerReportIdxControl->callback()(
      std::to_string(forwardWidgets.timerReportIdxControl->value()));
  forwardWidgets.totalTime->setValue(
      roundToDigits(forwardInfo.totalRuntime / 1000000.0, 5));
  forwardWidgets.iterTime->setValue(
      roundToDigits(forwardInfo.timer.totalMicroseconds / 1000000.0, 5));
}

void Viewer::addBackwardSimulationInfoWidets(
    std::string title, BackwardGradientInfoWidgets &infoWidgets,
    Simulation::BackwardInformation &gradientInfo,
    nanogui::ref<nanogui::Window> window, int widthOffset, bool isFiniteDiff) {
  using namespace nanogui;
  window = SettingsControl->addWindow(
      Eigen::Vector2i(width - widthOffset, height - 400), title);
  for (int i = 0; i < 2; i++) {
    infoWidgets.stiffness[i] =
        SettingsControl->addVariable((i == 0) ? "dL/dk_stretch" : "dL/dk_bend",
  ...
  for (int i = 0; i < 3; i++) {
    infoWidgets.f_ext[i] =
        SettingsControl->addVariable("", gradientInfo.dL_dfext[i], panel);
  ...
  for (int i = 0; i < 5; i++) {
    infoWidgets.f_wind[i] =
        SettingsControl->addVariable("", gradientInfo.dL_dwind[i], panel);
  ...
  for (int splineIdx = 0; splineIdx < 4; splineIdx++) {
    ...
    for (int i = 0; i < 9; i++) {
      infoWidgets.dL_dspline[splineIdx].emplace_back(
          SettingsControl->addVariable("", gradientInfo.dL_dsplines[0][0][0],
                                       panel));
  ...
  infoWidgets.density = SettingsControl->addVariable(
      "dL/ddensity", gradientInfo.dL_dk_pertype[3]);
  infoWidgets.x0 =
      SettingsControl->addVariable("||x0||", gradientInfo.dL_dk_pertype[3]);
  infoWidgets.mu =
      SettingsControl->addVariable("dL/mu", gradientInfo.dL_dk_pertype[3]);
  ...
  if (!isFiniteDiff) {
    infoWidgets.loss = SettingsControl->addVariable("loss", gradientInfo.loss);
    {
      ...
      infoWidgets.convergeIter = SettingsControl->addVariable(
          "iter", backwardInfo.backwardIters, panel);
      infoWidgets.accumConverge = SettingsControl->addVariable(
          "total converged", backwardInfo.convergedAccum, panel);
      infoWidgets.rho =
          SettingsControl->addVariable("rho", backwardInfo.rho, panel);
      infoWidgets.convergeIter->setFixedWidth(60);
      infoWidgets.accumConverge->setFixedWidth(60);
      infoWidgets.converged = SettingsControl->addVariable(
          "converged", forwardInfo.converged, panel);
...

    {
      ...
      infoWidgets.perfTimer[0] = SettingsControl->addVariable(
          "t_other",
          backwardInfo.timer.solvePerfReport.nonSolveTimeMicroseconds, panel);
      infoWidgets.perfTimer[1] = SettingsControl->addVariable(
          "t_direct",
          backwardInfo.timer.solvePerfReport.solveDirectMicroseconds, panel);
      infoWidgets.perfTimer[2] = SettingsControl->addVariable(
          "t_iter",
          backwardInfo.timer.solvePerfReport.solveIterativeMicroseconds, panel);
...

    {
      ...
      infoWidgets.timerReportIdxControl = SettingsControl->addVariable<int>(
          ...
          [&](const int &v) {
            if (v != std::clamp(
                         v, 0,
                         (int)gradientInfo.timer.timeMicroseconds.size() - 1)) {
              return;
            }
            infoWidgets.timerId = v;
            ...
            infoWidgets.timerReport->setValue(label + ":" + streamObj.str() +
                                              "|" + std::to_string(percentage) +
                                              "%");

            double accumDuration = gradientInfo.accumTimer[v].second;
            double accumTotal = 0;
            for (std::pair<std::string, long long> &item :
                 gradientInfo.accumTimer) {
              if ((item.first != "solveIterative") &&
                  (item.first != "solveDirect")) {
                accumTotal += item.second;
              }
      ...
      infoWidgets.totalTime = SettingsControl->addVariable(
          "t_total[s]", backwardInfo.totalRuntime, panel);
      formWidgetSetSpinnableWithIncrement(infoWidgets.timerReportIdxControl, 1);
    }
  }
}
...
void Viewer::gradientInfoWindowContentUpdate(
    BackwardGradientInfoWidgets &gradientWidgets,
  ...
  for (int i = 0; i < 2; i++)
    gradientWidgets.stiffness[i]->setValue(
        roundToDigits(gradient.dL_dk_pertype[2 + i], 5));
  ...
  int splineNum = std::min(gradient.dL_dsplines[sysMatId].size(),
                           gradientWidgets.dL_dspline.size());
  for (int i = 0; i < splineNum; i++) {
    for (int paramIdx = 0; paramIdx < gradient.dL_dsplines[sysMatId][i].rows();
         paramIdx++) {
  ...
  if (!isFiniteDiff) {
    ...
    gradientWidgets.timerReportIdxControl->callback()(
        std::to_string(gradientWidgets.timerReportIdxControl->value()));

    gradientWidgets.perfTimer[0]->setValue(
        gradient.timer.solvePerfReport.nonSolveTimeMicroseconds);
    gradientWidgets.perfTimer[1]->setValue(
        gradient.timer.solvePerfReport.solveDirectMicroseconds);
    gradientWidgets.perfTimer[2]->setValue(
        gradient.timer.solvePerfReport.solveIterativeMicroseconds);

    gradientWidgets.accumPerfTimer[0]->setValue(
        gradient.accumSolvePerformanceReport.nonSolveTimeMicroseconds);
    gradientWidgets.accumPerfTimer[1]->setValue(
        gradient.accumSolvePerformanceReport.solveDirectMicroseconds);
    gradientWidgets.accumPerfTimer[2]->setValue(
        gradient.accumSolvePerformanceReport.solveIterativeMicroseconds);
    gradientWidgets.totalTime->setValue(
        roundToDigits(gradient.totalRuntime / 1000.0, 2));
  }
}

nanogui::TextBox *createTextBox(nanogui::Widget *parent,
                                std::string defaultValue, double fontSize) {
  return new nanogui::TextBox(parent);
}

void Viewer::addParticleInfoWidgets() {
  particleInfoWindow = SettingsControl->addWindow(Eigen::Vector2i(300, 0),
                                                  "Particle&Triangle Info");
  using namespace nanogui;
  ...
  formWidgetSetSpinnableWithIncrement(particleInfoWidgets.particleIdxControl,
                                      1);

  Particle &p = simSystems[0]->particles[particleId];
  particleInfoWidgets.mass =
      SettingsControl->addVariable("mass", p.mass, panel);
  particleInfoWidgets.area =
      SettingsControl->addVariable("area", p.area, panel);
  ...
  particleInfoWidgets.neighborIdxToggle = SettingsControl->addVariable<int>(
      ...
      [&](const int &v) {
        if (particleId !=
            std::clamp(particleId, 0, (int)simSystems[0]->particles.size() - 1))
          return;
        Particle &p = simSystems[0]->particles[particleId];
        std::vector<int> &neighbors = simSystems[0]->particleTriangleMap[p.idx];
        if (v != std::clamp(v, 0, (int)neighbors.size() - 1))
          return;
  ...
  particleInfoWidgets.triangleNeighborFocus =
      new Button(panel, "", ENTYPO_ICON_AIR);
  particleInfoWidgets.triangleNeighborFocus->setCallback([&]() {
    ...
    Particle &p = simSystems[0]->particles[particleId];
    std::vector<int> &neighbors = simSystems[0]->particleTriangleMap[p.idx];
    if (particleInfoWidgets.neighborId !=
        std::clamp(particleInfoWidgets.neighborId, 0,
                   (int)neighbors.size() - 1))
      return;

    int neighborTriId = neighbors[particleInfoWidgets.neighborId];
    triangleInfoWidgets.triangleIdxControl->callback()(
        std::to_string(neighborTriId));
  });
  particleInfoWidgets.focus = SettingsControl->addButton("focus", [&]() {
    if (std::clamp(particleId, 0, ((int)simSystems[0]->particles.size()) - 1) !=
        particleId)
      return;
    Particle &p = simSystems[0]->particles[particleId];
    Vec3d pos;
    if (isforward) {
      std::vector<Simulation::ForwardInformation> forwardCopy =
          simSystems[0]->forwardRecords;
      if (std::clamp(playBackId, 0, ((int)forwardCopy.size()) - 1) !=
          playBackId)
        return;
      ...
      if ((!playGroundtruth) && (!playLineSearch)) {
        if (std::clamp(
                backwardSelectedRecordId, 0,
                ((int)simSystems[0]->backwardOptimizationRecords.size()) - 1) !=
            backwardSelectedRecordId)
          return;
        if (std::clamp(
                playBackId, 0,
        ...
        if (std::clamp(playBackId, 0,
                       ((int)simSystems[0]->groundTruthForwardRecords.size()) -
                           1) != playBackId)
          return;
        pos = simSystems[0]->groundTruthForwardRecords[playBackId].x.segment(
            p.idx * 3, 3);
      } else if (playLineSearch) {
        if (std::clamp(playBackId, 0,
            ...
            playBackId)
          return;
        pos = simSystems[0]->linesearchRecords[playBackId].x.segment(p.idx * 3,
                                                                     3);
      }
    }
  ...
  formWidgetSetSpinnableWithIncrement(triangleInfoWidgets.triangleIdxControl,
                                      1);
  ...
  triangleInfoWidgets.area =
      SettingsControl->addVariable("area", t.area_rest, panel);
  panel = new Widget(particleInfoWindow);
  ...
  for (int i = 0; i < 3; i++) {
    triangleInfoWidgets.particleId[i] =
        SettingsControl->addVariable("", t.idxArr[i], panel);
    triangleInfoWidgets.focusButtons[i] =
        new Button(panel, "", ENTYPO_ICON_AIRCRAFT);
    triangleInfoWidgets.focusButtons[i]->setCallback([this, i]() {
      if (std::clamp(triangleId, 0, (int)simSystems[0]->mesh.size()) !=
          triangleId)
        return;
      Triangle &t = simSystems[0]->mesh[triangleId];
      particleId = t.idxArr[i];
      particleInfoWidgets.particleIdxControl->callback()(
  ...
  triangleInfoWidgets.focus = SettingsControl->addButton("focus", [&]() {
    ...
    if (isforward) {
      if (std::clamp(playBackId, 0,
                     ((int)simSystems[0]->forwardRecords.size()) - 1) !=
          playBackId)
        return;
      pos =
          simSystems[0]->forwardRecords[playBackId].x.segment(t.p0_idx * 3, 3);
    } else {
      if (!playGroundtruth) {
        if (std::clamp(
                backwardSelectedRecordId, 0,
...
void Viewer::updateParticleInfoContent() {
  ...
  if (isforward) {
    ...
    if ((!playGroundtruth) && (!playLineSearch)) {
      if (std::clamp(backwardSelectedRecordId, 0,
                     ((int)simSystems[0]->backwardOptimizationRecords.size()) -
                         1) != backwardSelectedRecordId)
        proceed = false;
  ...
  if ((particleId >= 0) && (particleId < simSystems[0]->particles.size())) {
    Particle &p = simSystems[0]->particles[particleId];
    std::printf(
        "selected particle %d: mass:%.3f area: %.3f neighborhoodsize: %zu\n",
        p.idx, p.mass, p.area,
    ...
    if (particleInfoWidgets.neighborId ==
      ...
      particleInfoWidgets.triangles->setValue(
          std::to_string(neighbors[particleInfoWidgets.neighborId]));
    }
...
void Viewer::addBackwardSimulationControlWidets() {
  using namespace nanogui;
  backwardSimControlWindow = SettingsControl->addWindow(
      Eigen::Vector2i(width - 350, 200), "Backward Cloth Simulation");
  ...
  perfResolutionWidget = SettingsControl->addVariable<int>(
      "Perf resoution", [&](const int &v) { perfResolution = v; },
  ...
  SettingsControl->addVariable<bool>(
      "Finite diff", [&](const bool &v) { useFiniteDiff = v; },
      [&]() -> bool { return useFiniteDiff; });
  SettingsControl->addVariable<bool>(
      "LBfGS", [&](const bool &v) { useLBFGS = v; },
      [&]() -> bool { return useLBFGS; });

  SettingsControl->addVariable<bool>(
      "Rand init", [&](const bool &v) { randomInit = v; },
  ...
  { // Backward select records controls, slider+textbox+two buttons
    ...
    backwardRecordSliderText.slider->setFinalCallback([&](float value) {
      ...
      floatToIdxAndText(value, backwardRecords.size(), backwardSelectedRecordId,
                        recordText);
    ...
    backwardRecordSliderText.idxWidget = SettingsControl->addVariable<int>(
        ...
        [&](const int &v) {
          ...
          if (backwardSelectedRecordId != prevIdx) {
            double sliderVal;
            std::string recordText;
            idxToFloatAndText(backwardSelectedRecordId, totalRecords, sliderVal,
                              recordText);
  ...
  { // Backward select frame control
    ...
    backwardFrameSliderText.textbox->setValue(
        simSystems[0]->backwardOptimizationRecords.size() == 0
            ? "0/0"
            : "1/" + std::to_string(simSystems[0]
                                        ->backwardOptimizationRecords[0]
                                        .first.size()));
    backwardFrameSliderText.slider->setFinalCallback([&](float value) {
      ...
      if ((!playGroundtruth) && (!playLineSearch)) {
        //          std::printf(
        //                  "trying to update actual gradient ...
        //                  totalBackwardRecordNum: %zu select:%d totalFrames:
        ...
        backwardInfo =
            simSystems[0]
                ->backwardOptimizationRecords[backwardSelectedRecordId]
                .second[playBackId];
        gradientInfoWindowContentUpdate(gradientInfoWidgets, backwardInfo,
                                        false);
        forwardInfoWindowContentUpdate(forwardInfoWidgets, forwardInfo);

        if (useFiniteDiff) {

          if ((simSystems[0]->backwardRecordsFiniteDiff.size() >
               backwardSelectedRecordId) &&
              (simSystems[0]
                   ->backwardRecordsFiniteDiff[backwardSelectedRecordId]
                   .size() > playBackId)) {
            finitediffGradientInfo =
                simSystems[0]
                    ->backwardRecordsFiniteDiff[backwardSelectedRecordId]
                                               [playBackId];
            gradientInfoWindowContentUpdate(finiteDiffInfoWidgets,
                                            finitediffGradientInfo, true);
          } else {
            std::printf("ERROR: want to display finite diff info but there is "
                        "record number mismatch\n");
            std::printf(
                "trying to update actual gradient ...  "
                "totalFiniteDiffRecordNum: %zu select:%d  , selectId: %d\n",
                simSystems[0]->backwardRecordsFiniteDiff.size(),
                backwardSelectedRecordId, playBackId - 1);
          }
        }
  ...
  backwardPlayGroundtruthSimulationButton =
      SettingsControl->addButton("Play groundtruth", [&]() {
        isplaybackSimulationOn = true;
  ...
  nanogui::Button *playLineSearchButton =
      SettingsControl->addButton("Play linesearch", [&]() {
        isplaybackSimulationOn = true;
        playLineSearch = true;
        playGroundtruth = false;
        playBackId = 0;
...
void Viewer::addForwardSimulationControlWidets() {
  using namespace nanogui;
  forwardSimControlWindow = SettingsControl->addWindow(
      Eigen::Vector2i(width - 250, 0), "Forward Cloth Simulation");
  ...
  forwardStartSimulationButton = SettingsControl->addButton(
      "Start Simulation", [&]() { isSimulationOn = true; });
  forwardStopSimulationButton = SettingsControl->addButton(
      "Stop Simulation", [&]() { isSimulationOn = false; });
  playbackSimulationButton =
      SettingsControl->addButton("Playback Simulation", [&]() {
        isplaybackSimulationOn = true;
        playBackId = 0;
  ...
  forwardFrameSliderText.textbox->setValue(
      "1/" + std::to_string(simSystems[0]->forwardRecords.size()));
  forwardFrameSliderText.textbox->setUnits("");
  forwardFrameSliderText.slider->setCallback([&](float value) {
    if (!isforward)
      return;
    std::vector<Simulation::ForwardInformation> forwardRecordCopy =
        simSystems[0]->forwardRecords;
  ...
  //
  //      std::string folder = std::string(SOURCE_PATH) +
  //      fileNameTextBox->value(); frameFileNames = listFiles(folder, ".obj");
  //
  //      std::sort(frameFileNames.begin(), frameFileNames.end());
  //
  //      simSystems[0]->loadWindSim2RealAnimationSequence(selectedFolder,
  //      frameFileNames,
  //                                                       selectedFolder ==
  //                                                       "flag-ryanwhite");
  //      camera.setCamPos(toGlm(simSystems[0]->sceneConfig.camPos));
  //      camera.setLookAt(Simulation::getLookAtPos(simSystems[0],
  //      simSystems[0]->sceneConfig)); isSimulationOn = false; isforward =
  //      true; isplaybackSimulationOn = false;
  //
  //  });
}
...
void Viewer::addReinitWidgets() {
  reinitWindow =
      SettingsControl->addWindow(Eigen::Vector2i(30, 500), "Reinit Settings");
  ...
  densityControl =
      SettingsControl->addVariable<double>("density", density, panel);
  densityControl->setFixedWidth(60);
  formWidgetSetSpinnableWithIncrement(dimControl, 1);

  // stretching
  strechingStiffnessControl = SettingsControl->addVariable<double>(
      "k_stretch", strechingStiffness, panel);
  strechingStiffnessControl->setFixedWidth(60);
  formWidgetSetSpinnableWithIncrement(strechingStiffnessControl, 5.0);
  // bending
  bendingStiffnessControl =
      SettingsControl->addVariable<double>("k_bend", bendingStiffness, panel);
  ...
  // load animations
  animationOptions =
      listDirectory(std::string(SOURCE_PATH) + "/src/assets/animation/");
  ...
  for (Simulation::SceneConfiguration &s :
       OptimizationTaskConfigurations::sceneConfigArrays) {
    demoOptions.push_back(s.name);
  }
  SettingsControl->addVariable("DemoConfig", currentSceneConfig, true)
      ->setItems(demoOptions);

  SettingsControl->addButton("WindSim2Real", [&]() {
    std::vector<std::string> frameFileNames;
    std::string selectedFolder =
        animationOptions[boxAnimationFiles->selectedIndex()];
    std::string folder =
        std::string(SOURCE_PATH) + "/src/assets/animation/" + selectedFolder;
