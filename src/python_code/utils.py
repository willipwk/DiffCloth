...
def plotPointCloud(pointMat):  # should be   #numPoints x 3
    if pointMat.shape[1] != 3:
        print(
            "Trying to plot point cloud. expect matrix size to be (#numPoints, 3), but instead get {}".format(
                pointMat.shape
            )
        )
    x = pointMat[:, 0]
...
def plotPointCloudFromVec(x_vec):
    ...
    ax.view_init(elev=10.0, azim=10)
...
def getPointOnSphere(
    radius, phi, theta
):  # phi: changes x,z  theta: changes y (up and down)
    p = radius * np.array(
        [
