...
from diffcloth_py import ForwardInformation, Simulation
...
class SimFunction(autograd.Function):
    ...
    @staticmethod
    def backward(
        ctx: Any, dL_dx_next: Tensor, dL_dv_next: Tensor
        ...
        if isLast:
            backRecord = cppSim.stepBackwardNN(
                ...
                ctx.newRecord,
                ctx.newRecord.stepIdx
                == 1,  # TODO: check if this should be 0 or 1
                dL_dxnew_np,
                dL_dvnew_np,
            )
        else:
        ...
        if dL_da_norm > 1e-7:
            ...
            normalized = backRecord.dL_dxfixed * (
                max(
                    min(backRecord.dL_dxfixed.shape[0] * maxNorm, dL_da_norm),
                    0.05,
                )
                / dL_da_norm
            )
