...
def getX0A0PairsFromSphericalCoord(xzDegree, yDegree):
    ...
    xmean_newPoint = (
        utils.getPointOnSphere(dist, xzRad, math.radians(yDegree))
        + HEAD_CENTER_POS
    )
    ...
    a0_shifted = common.toTorchTensor(
        CLIP_INIT_POS + np.tile(translation, (1, ndof_u // 3)), True, False
    )
...
def getX0A0PairsFromRange(num=10, yRangeDegree=(0, 90), xzRangeDegree=(0, 360)):
    ...
    pairs = []
    for i in range(num):
        xzDegree, yDegree = random.randrange(xzMin, xzMax), random.randrange(
            yMin, yMax
        )
        print(
            "Generating {}-th position at (xz, y)=({},{}) deg".format(
                i, xzDegree, yDegree
            )
        )
        (x0_shifted, a0_shifted) = getX0A0PairsFromSphericalCoord(
            xzDegree, yDegree
        )
...
def lossFunction(xvPairs):
    ...
    for (i, (x_i, v_i)) in enumerate(xvPairs):
        ...
        stretchPenalty += (
            torch.clamp(
                torch.abs(clip_dist - CLIP_REST_DIST) - 1.0, min=0.0, max=None
            )
            * 0.2
        )
    ...
    for (x_last, v_i) in xvPairs:
        for (i, (idx1, idx2)) in enumerate(CLIP_DIR_VERTEX_PAIR):
            dir = (
                x_last[idx1 * 3 : idx1 * 3 + 3]
                - x_last[idx2 * 3 : idx2 * 3 + 3]
            )
            dirGoal = (
                targetShape[idx1 * 3 : idx1 * 3 + 3]
                - targetShape[idx2 * 3 : idx2 * 3 + 3]
            )
            dirGoaltorch = common.toTorchTensor(dirGoal, False, False)
            cosine = torch.clamp(
                torch.dot(
                    torch.nn.functional.normalize(dir, dim=0),
                    torch.nn.functional.normalize(dirGoaltorch, dim=0),
                ),
                max=0.5,
                min=None,
            )
        ...
        targetLoss += torch.nn.functional.smooth_l1_loss(
            x_last, targetshape_torch
        )
    targetLast = torch.nn.functional.smooth_l1_loss(
        xvPairs[-1][0], targetshape_torch
    )
...
def simulateAndGetLoss(x0a0pairs, render=True):
    ...
    losses = []
    print(
        "[simulationAndGetLoss] totalSeq: {} running simulation...".format(
            len(x0a0pairs)
        )
    )
    ...
    for (i, (x0_shift_torch, a0_shift_torch, _)) in enumerate(x0a0pairs):
        sim.resetSystem()
        xvPairs = common.forwardSimulation(
            sim,
            x0_shift_torch.clone(),
            v0_torch.clone(),
            a0_shift_torch.clone(),
            getState,
            controller,
            pySim,
        )
...
def getValidationLosses(
    epoch, expName, render=False, saveImage=True, saveSimulation=True
):
    ...
    lossAvg, losses, simulations = simulateAndGetLoss(
        X0A0pairs_eval, render=render
    )

    for (i, (x0a0Deg, loss, xvPairs)) in enumerate(
        zip(X0A0pairs_eval, losses, simulations)
    ):
        print(
            "{}/{}: {}".format(
                i + 1,
                len(X0A0pairs_eval),
                "Succeed" if loss["succeed"].item() else "Failed",
            )
        )
        degrees = x0a0Deg[2]
        success_count += int(loss["succeed"])

        clothVec, clothVecInit = (
            xvPairs[-1][0].detach().numpy(),
            xvPairs[0][0].detach().numpy(),
        )
        vecStack = np.concatenate([clothVec, clothVecInit], axis=0)
        if saveImage:
            Path(root_path / "evalImages" / expName).mkdir(
                parents=True, exist_ok=True
            )
            identifier = "epoch-{:d}-deg{:.1f}-{:.1f}-loss-{:.2f}".format(
                epoch, degrees[0], round(degrees[1]), loss["total"].item()
            )
            savePath = root_path / "evalImages" / expName / identifier
            print("saving image to... {}".format(savePath))

            utils.plotPointCloudFromVecs(
                [vecStack, headVec], identifier, save=saveImage, path=savePath
            )
        if saveSimulation:
            savePath = (
                expName + "_eval" + "_{}_{}".format(degrees[0], degrees[1])
            )
...
def getState(x, v):
    HEAD_CENTER_POS = common.toTorchTensor(
        sim.primitives[0].center.copy(), False, False
    )
    headRadius = 2.1

    state = [x - targetshape_torch]
    v_mean, x_mean = v.reshape(-1, 3).mean(axis=0), x.reshape(-1, 3).mean(
        axis=0
    )
    elevationVector = headRadius * torch.nn.functional.normalize(
        x_mean - HEAD_CENTER_POS, dim=0
    )
...
def loadCheckpoint(path, epoch):
    ...
    epochStart, testMinLoss, trainMinLoss, trainLosses, testLosses = (
        checkpoint["epoch"],
        checkpoint["testMinLoss"],
        checkpoint["trainMinLoss"],
        checkpoint["trainLosses"],
        checkpoint["testLosses"],
    )
    successLog, trainBestEpoch, testBestEpoch = (
        checkpoint["successLog"],
        checkpoint["trainBestEpoch"],
        checkpoint["testBestEpoch"],
    )
    print(
        "Loaded checkpoint from epoch {}, trainMinLoss is {}".format(
            epochStart, trainMinLoss
        )
    )
...
controller = IndClosedController(
    sim,
    helper,
    [getState(x0_torch, v0_torch).size(1), 64, 64, ndof_u],
    dropout=0.0,
)
...
if not (eval_mode):
    ...
    if not (train_resume):
        ...
        os.system(
            "cp hatController.py {}".format(
                exp_path / "wearhat_nn_{}.py".format(dt_string)
            )
        )
        os.system(
            "cp utils.py {}".format(exp_path / "utils_{}.py".format(dt_string))
        )
        os.system(
            "cp closedLoop_common.py {}".format(
                exp_path / "closedLoop_common_{}.py".format(dt_string)
            )
        )
        ...
        (
            ...
            trainBestEpoch,
            testBestEpoch,
        ) = loadCheckpoint(exp_path, "{}.pth".format(loadEpoch))
    ...
    for epoch in range(epochStart, args.epochNum):
        ...
        loss = trainStep(
            sim,
            optimizer,
            trainSeqSampleNum=20,
            render=args.render and (epoch > 0) and (epoch % 10 == 0),
        )
        ...
        utils.log(
            "Train: loss: {} minLoss: {} bestEpoch: {} norm:{} \n".format(
                loss,
                trainMinLoss,
                trainBestEpoch,
                nn.utils.clip_grad_norm_(controller.parameters(), 1.0),
        ...
        evalLoss, evalSuccessNum = getValidationLosses(
            epoch,
            expName,
            render=(epoch % 10 == 0) and (args.render),
            saveImage=True,
        ...
        utils.log(
            "Test: loss: {} minLoss: {} bestEpoch:{}, evalSuccessNum:{}/{} rate:{}\n".format(
                ...
                evalSuccessNum,
                len(X0A0pairs_eval),
                evalSuccessNum * 1.0 / len(X0A0pairs_eval),
            ),
            not (eval_mode),
            logFile,
        )
