...
def render_thread(perturbed_obj_fn, urdf_fn, mode="GUI"):
    ...
    if "grasp1" in perturbed_obj_fn:
        kp_indices = [347, 347]
    elif "grasp2" in perturbed_obj_fn:
        kp_indices = [14, 14]
    elif "grasp3" in perturbed_obj_fn:
        kp_indices = [251, 251]
    elif "grasp4" in perturbed_obj_fn:
        kp_indices = [269, 269]
    elif "grasp5" in perturbed_obj_fn:
        kp_indices = [266, 131]
    elif "grasp6" in perturbed_obj_fn:
        kp_indices = [251, 341]
    ...
    with open(f"{out_fn}_pos.npy", "wb") as fp:
    ...
    # check file
    if not (
        os.path.isfile(f"{out_fn}.ply") or os.path.isfile(f"{out_fn}_pos.npy")
    ):
...
if __name__ == "__main__":
    fn = sys.argv[1]
    mode = sys.argv[2]
    assert mode in ["GUI", "DIRECT"], "Wrong rendering mode"
    render_thread(fn, "tie/tie.urdf", mode)
